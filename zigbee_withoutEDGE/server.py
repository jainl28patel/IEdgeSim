...
class Coordinator(Node):
   ...
   def route_message(self, sender_name, recipient_address, message):
       sender = next(node for node in self.network.nodes if node.name == sender_name)
       recipient = next(node for node in self.network.nodes if node.address == recipient_address)
